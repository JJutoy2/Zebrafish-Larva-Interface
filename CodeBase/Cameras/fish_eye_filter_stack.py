...
def filter_stack_eye_angle(stream_vars: Tuple[np.ndarray,int], w: WindowParameters, d: CamData):
    '''
    ...
    eye_angle = find_eye_angle(img,bw,d.filter_data.bb, d.filter_data.fish_eye)
    #TODO Depending on eye determine which angle set to use?

    output =    {'eye_angle': eye_angle,
                'cam_time': w.frame_time.current_time-w.frame_time.start,
                }
    d.set_output(output) 
